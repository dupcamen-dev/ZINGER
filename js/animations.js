document.addEventListener('DOMContentLoaded', () => {
    ...
    init_animations = () => {
        ...
        if(document.querySelector('.mod-scroll__intro')){
            scroll_intro_tl.timeScale(1000).play()       
        }
    ...
    last_animations = () => {
        ...
        if(document.querySelectorAll('.mod-footer').length > 0){
            ...
            document.querySelectorAll('.mod-footer').forEach( (elem) => {
                                ...
                                ///footer__bg
                const footer_logo = elem.querySelector('.mod-footer__bg')
                const footer_logo_tl = gsap.timeline({paused:true})
                footer_logo_tl.from(footer_logo, { opacity: 0, yPercent: 30, duration: 1.4, ease: 'power2.out' },0)
                ScrollTrigger.create({
